...
@Component({
    selector: 'jhi-payment-method-my-suffix',
    templateUrl: './payment-method-my-suffix.component.html'
})
export class PaymentMethodMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/payment-method-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/payment-method-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PaymentMethodMySuffix) {
        return item.id;
    }
