...
@Component({
    selector: 'jhi-person-address-my-suffix',
    templateUrl: './person-address-my-suffix.component.html'
})
export class PersonAddressMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/person-address-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/person-address-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PersonAddressMySuffix) {
        return item.id;
    }
