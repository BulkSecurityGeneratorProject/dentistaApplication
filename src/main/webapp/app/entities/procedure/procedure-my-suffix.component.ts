...
@Component({
    selector: 'jhi-procedure-my-suffix',
    templateUrl: './procedure-my-suffix.component.html'
})
export class ProcedureMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/procedure-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/procedure-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ProcedureMySuffix) {
        return item.id;
    }
