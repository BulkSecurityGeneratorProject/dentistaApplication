...
@Component({
    selector: 'jhi-appointment-my-suffix',
    templateUrl: './appointment-my-suffix.component.html'
})
export class AppointmentMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/appointment-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/appointment-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AppointmentMySuffix) {
        return item.id;
    }
